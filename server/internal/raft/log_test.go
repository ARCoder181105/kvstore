...
func TestGetLastTerm_WithEntries(t *testing.T) {
	node := &RaftNode{
		log: []LogEntry{{Index: 0, Term: 1}, {Index: 1, Term: 5}},
	}
...
func TestGetLastTerm_AfterTruncate(t *testing.T) {
	node := &RaftNode{
		log: []LogEntry{{Index: 0, Term: 1}, {Index: 1, Term: 2}, {Index: 2, Term: 3}},
	}
	node.truncateFrom(2) // Log becomes [ {Index:0, Term:1}, {Index:1,Term:2} ]
...
func TestGetEntry_ValidIndex(t *testing.T) {
	...
	node := &RaftNode{
		log: []LogEntry{{Index: 0, Term: 1}, expected},
	}
...
func TestAppendEntry_FreshNode(t *testing.T) {
	node := &RaftNode{}
	node.appendEntry(LogEntry{Index: 0, Term: 1})
	if len(node.log) != 1 || node.log[0].Term != 1 {
		t.Errorf("Failed to append to fresh node")
	}
}

func TestAppendEntry_ExistingEntries(t *testing.T) {
	node := &RaftNode{log: []LogEntry{{Index: 0, Term: 1}}}
...
func TestTruncateFrom_Middle(t *testing.T) {
	node := &RaftNode{
		log: []LogEntry{{Index: 0, Term: 0}, {Index: 1, Term: 1}, {Index: 2, Term: 2}},
	}
...
func TestTruncateFrom_Full(t *testing.T) {
	...
	node.truncateFrom(0) // Should remove everything
	if len(node.log) != 0 {
		t.Errorf("Expected length 0, got %d", len(node.log))
	}
}
...
func TestTruncateFrom_FreshNode(t *testing.T) {
	node := &RaftNode{}
	node.truncateFrom(0) // Should be no-op on empty log without panic
	if len(node.log) != 0 {
		t.Errorf("Expected length 0, got %d", len(node.log))
	}
}
